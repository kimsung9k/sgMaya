...
class Function_skinWeight:
    ...
    def __init__(self, srcMesh, influences ):
        ...
        self.influenceIndices = [ None for i in range( len( influences ) ) ]
        for origPlug, influencePlug in pymel.core.listConnections( fnSkinCluster.name() + '.matrix', s=1, d=0, p=1, c=1 ):
            ...
            self.influenceIndices[ influences.index(influenceName) ] = origPlug.index()
    ...
    def copyWeight( src, trg ):
        ...
        for srcInfluence in srcInfluences:
            weightValue = src.weightsPlug.elementByLogicalIndex(srcInfluence).asFloat()
            weightsList.append( weightValue )
            otherSumWeights -= weightValue

        trgTargetPlugs    = []
        trgSetWeightsValues = []
        trgOtherPlugs = []
        trgOtherSumWeights = 1.0
        for trgInfluence in trgInfluences:
            trgOtherSumWeights -= trg.weightsPlug.elementByLogicalIndex(trgInfluence).asFloat()

        for i in range( trg.weightsPlug.numElements() ):
            if trg.weightsPlug[i].logicalIndex() in trgInfluences:
                index = trgInfluences.index( trg.weightsPlug[i].logicalIndex() )
                trgTargetPlugs.append(trg.weightsPlug[i])
                trgSetWeightsValues.append(weightsList[index])
            else:
                trgOtherPlugs.append(trg.weightsPlug[i])
                trgOtherSumWeights -= trg.weightsPlug[i].asFloat()

        if len( trgOtherPlugs ):
            if trgOtherSumWeights  == 0:
                multValue = 0
            else:
                multValue = otherSumWeights/trgOtherSumWeights
            for i in range( len( trgOtherPlugs ) ):
                cmds.setAttr( trgOtherPlugs[i].name(), trgOtherPlugs[i].asFloat() * multValue )

        for i in range( len( trgTargetPlugs ) ):
            cmds.setAttr( trgTargetPlugs[i].name(),  trgSetWeightsValues[i] )
